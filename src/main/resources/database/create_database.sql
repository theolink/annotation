...
(
  id       BIGINT(20) PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  trial_id BIGINT(20) COMMENT '庭审ID',
  name VARCHAR(30) COMMENT '原始视频名称',
  address  VARCHAR(256) COMMENT '原始视频地址',
  uploader  BIGINT(20) COMMENT '上传者ID',
  uploadDate datetime DEFAULT CURRENT_TIMESTAMP COMMENT '上传时间',
  preDeal INT(1) DEFAULT 0 COMMENT '视频预处理状态,0：未处理，1：处理中，2：已处理',
  preDealer BIGINT(20) COMMENT '预处理执行者ID',
  preDealDate datetime  COMMENT '预处理执行时间时间',
  is_deleted  INT(1) DEFAULT 0 COMMENT '是否删除，0：未删除；1：已删除'
);